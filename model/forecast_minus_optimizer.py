"""
Title: forecast_optimizer_minus.py
Description: The semi-supervised optimizer with minus loss function for anomaly.
Author: Lek'Sai Ye
...
# 3.2. (1) Trainer
# --------------------------------------------
class ForecastTrainerMinus(BaseTrainer):
    def __init__(self,
                 eta: float= 1,
    ...
    def train(self, dataset, net, bound=1):
        ...
        for epoch in range(self.n_epochs):
            ...
            for data in train_loader:
                ...
                losses = torch.where(y == 0,
                                     dist_mean,
                                     torch.where(dist_mean * (-1) < - bound,
                                                 - bound * torch.ones_like(dist_mean,
                                                                           device=self.device),
                                                 dist_mean * (-1)))
    ...
    def test(self, dataset, net, bound=1):
...
# 3.2. (b) Evaluater
# --------------------------------------------
class ForecastEvaluaterMinus(BaseEvaluater):
    def __init__(self,
                 eta: float = 1,
                 batch_size: int = 32,
                 device: str = 'cuda:1',
    ...
    def test(self, dataset, net, bound):
